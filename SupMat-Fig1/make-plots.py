...
plt.figure(figsize=(3.5,2.7), dpi=150)
plt.plot(f_GW_init_range, gpudat2d[5,:].T * 1e6)
plt.xscale('log')
plt.xlabel(r"Initial GW frequency, $f_\mathrm{in}$ [Hz]")
plt.ylabel('Time [µs]')
plt.savefig("timings-1d-slice.pdf", bbox_inches='tight')
plt.close()

plt.figure(figsize=(3.5,2.7), dpi=150)
plt.pcolormesh(Mc_range, f_GW_init_range, cpudat2d.T, rasterized=True)
plt.xscale('log')
plt.yscale('log')
plt.xlabel(r"Chirp mass, $\mathcal{M}_c$ [$M_\odot$]")
plt.ylabel(r"Initial GW frequency, $f_\mathrm{in}$ [Hz]")
cbar = plt.colorbar(label="GPU statistic cost [s]")
plt.savefig("timings-2d-cpu.pdf", bbox_inches='tight')
plt.close()

plt.figure(figsize=(3.5,2.7), dpi=150)
plt.plot(f_GW_init_range, cpudat2d[5,:].T)
plt.xscale('log')
plt.xlabel(r"Initial GW frequency, $f_\mathrm{in}$ [Hz]")
plt.ylabel('Time [s]')
plt.savefig("timings-1d-slice-cpu.pdf", bbox_inches='tight')
plt.close()
...
for (ix, iy, c, ls) in zip(inds_x, inds_y, colors, lses):
    ...
    ax.loglog(n_stat, gpudat[ix,iy], c=c, ls=ls, lw=1.2, label=label)
    ax.axhline(cpudat[ix,iy], c=c, ls='--', lw=1.2)
    print(ix, iy, cpudat[ix,iy])
ax.legend(frameon=False, alignment="left", title_fontsize=10, loc="lower left", title=r'$\mathcal{M}_c [M_\odot], f_\mathrm{in}[\mathrm{mHz}]$')
